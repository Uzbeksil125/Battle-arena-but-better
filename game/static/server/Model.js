...
let lava = new Terrain(2, 'lava', 5, 1);
...
class Map {
  constructor() {
    ...
    for (let i = 0; i < this.heightInSquares; i++) {
      ...
      for (let j = 0; j < this.widthInSquares; j++) {
        if (i == 0 || j == 0 || i == this.heightInSquares - 1 || j == this.widthInSquares - 1)
          this.square[i][j] = edge;
        else
          this.square[i][j] = grass;
      }
    }
    // Increased sizes for visibility on 100x100 map (10,000 tiles total)
    this.createArea(new Point(10, 10), 1000, 0.75, sand);
    this.createArea(new Point(80, 20), 1200, 0.75, water);
    this.createArea(new Point(50, 50), 800, 0.65, lava);
    this.createArea(new Point(20, 80), 1000, 0.75, sand);
    this.createArea(new Point(80, 80), 1000, 0.75, water);

    // Adjusted/Removed out of bounds buildings/walls
    this.createBuilding(60, 60, 10, 10, 2);
    this.createBuilding(20, 20, 15, 20, 3);
    this.createBuilding(40, 40, 10, 10, 1);

    // DEBUG: Count terrain types
    let counts = {};
    for (let r = 0; r < this.heightInSquares; r++) {
      for (let c = 0; c < this.widthInSquares; c++) {
        let t = this.square[r][c].type;
        counts[t] = (counts[t] || 0) + 1;
      }
    }
    console.log("Map Generation Complete. Counts:", counts);
  }
  ...
  createArea(center, size, randomness, type) { ///using a DFS algorithm
    ...
    while (queue.length != 0 && currentSize < size) {
      ...
      if (!this.square[current.y] || !this.square[current.y][current.x]) {
        console.error("Invalid Coordinate:", current.x, current.y);
        continue;
      }
      ...
      // Check Right (x + 1)
      if (Math.random() <= randomness && current.x + 1 >= 1 && current.x + 1 <= 98 && current.y >= 1 && current.y <= 98 && this.square[current.y][current.x + 1] !== type) {
        let currentNew = new Point(current.x + 1, current.y);
        queue.push(currentNew);
      }
      // Check Down (y + 1)
      if (Math.random() <= randomness && current.x >= 1 && current.x <= 98 && current.y + 1 >= 1 && current.y + 1 <= 98 && this.square[current.y + 1][current.x] !== type) {
...
class Items {
  constructor(mapSquares) {
    this.array = [];
    this.widthInSquares = mapSquares[0].length; // Hacky inference but effective if mapSquares is square[][]
    if (!this.widthInSquares) this.widthInSquares = 400; // Fallback
    this.heightInSquares = mapSquares.length;
  ...
  generateItems(amount, mapSquares) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      let newX, newY;
      do {
        newX = Math.random() * this.widthInSquares * 50;
        newY = Math.random() * this.heightInSquares * 50;
      } while (!mapSquares[Math.floor(newY / 50)][Math.floor(newX / 50)].isPassable);
